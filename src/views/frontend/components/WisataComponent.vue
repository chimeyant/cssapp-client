<template>
  <v-row>
    <v-col cols="12">
      ...
      >
        <v-container>
          <div class="lokasi-title">DAFTAR OBJEK WISATA DI KABUPATEN TANGERANG</div>
          <div>
    ...
    <v-col>
      ...
      >
        ...
        >
          ...
          >
          <div class="orange darken-2 slider-title-item ">
            {{ slide.name }}
          </div>
        </slide>
      </carousel-3d>
...
export default {
  ...
  methods: {
    ...
    fetchLokasi: async function () {
      try {
        let { data } = await this.http.get("api/daftar-wisata");
        this.records = data;
        setTimeout(function () {
          window.dispatchEvent(new Event("resize"));
        }, 250);
        window["carousel-3d"].Slide;
      } catch (error) {}
    },
    postShowLokasi: async function (val) {
      try {
        let { data } = await this.http.get("api/show-wisata/" + val);
        this.record = data;
